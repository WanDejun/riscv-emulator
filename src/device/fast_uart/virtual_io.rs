use crate::{cli_coordinator::CliCoordinator, device::fast_uart::UartIOChannel};
use clap::ValueEnum;
...
pub trait SerialDestTrait {
    // fn new(channel: UartIOChannel) -> Self;
}
...
impl TerminalIO {
    pub fn new(channel: UartIOChannel) -> Self {
        spawn_io_thread(channel.input_tx, channel.output_rx, channel.busy);
        Self {}
    }
}
