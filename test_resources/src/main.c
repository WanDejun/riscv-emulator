...
extern char starttext[], endtext[], startrodata[], endrodata[], startdata[],
    enddata[], startbss[], endbss[], stack_lower_bound[], stack_top[];

void display_section_info() {
  Log(INFO, ".text section: [%08x, %08x]", (uintptr_t)(starttext),
      (uintptr_t)endtext);
  Log(INFO, ".rodata section: [%08x, %08x]", (uintptr_t)startrodata,
      (uintptr_t)endrodata);
  Log(INFO, ".data section: [%08x, %08x]", (uintptr_t)startdata,
      (uintptr_t)enddata);
  Log(INFO, ".bas section: [%08x, %08x]", (uintptr_t)startbss,
      (uintptr_t)endbss);
