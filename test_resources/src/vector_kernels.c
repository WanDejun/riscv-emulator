...
static int
check_i32(const char* name, const int32_t* got, const int32_t* expected, size_t n) {
...
static int
check_i16(const char* name, const int16_t* got, const int16_t* expected, size_t n) {
    for (size_t i = 0; i < n; ++i) {
        if (got[i] != expected[i]) {
            printf(
                "%s mismatch at %ld: got %ld expected %ld\n", name, (long)i, (long)got[i],
...
static void
saxpy_ref(int32_t* dst, const int32_t* x, const int32_t* y, int32_t a, size_t n) {
    for (size_t i = 0; i < n; ++i) {
        dst[i] = a * x[i] + y[i];
    }
}
...
static void
gemm_ref(int32_t* c, const int32_t* a, const int32_t* b, size_t m, size_t n, size_t k) {
...
static void
gemm_rvv_i32(int32_t* c, const int32_t* a, const int32_t* b, size_t m, size_t n, size_t k) {
...
static int run_instruction_smoke(void) {
    ...
    for (size_t i = 0; i < N; ++i)
        expected[i] = a[i] + b[i];
    if (!check_i32("vadd.vv", got, expected, N))
        return 0;

    rvv_sub_vv_i32(got, a, b, N);
    for (size_t i = 0; i < N; ++i)
        expected[i] = a[i] - b[i];
    if (!check_i32("vsub.vv", got, expected, N))
        return 0;

    rvv_and_vv_i32(got, a, b, N);
    for (size_t i = 0; i < N; ++i)
        expected[i] = a[i] & b[i];
    if (!check_i32("vand.vv", got, expected, N))
        return 0;

    rvv_or_vv_i32(got, a, b, N);
    for (size_t i = 0; i < N; ++i)
        expected[i] = a[i] | b[i];
    if (!check_i32("vor.vv", got, expected, N))
        return 0;

    rvv_xor_vv_i32(got, a, b, N);
    for (size_t i = 0; i < N; ++i)
        expected[i] = a[i] ^ b[i];
    if (!check_i32("vxor.vv", got, expected, N))
        return 0;

    rvv_sll1_vi_i32(got, a, N);
    for (size_t i = 0; i < N; ++i)
        expected[i] = a[i] << 1;
    if (!check_i32("vsll.vi", got, expected, N))
        return 0;

    rvv_sra1_vi_i32(got, a, N);
    for (size_t i = 0; i < N; ++i)
        expected[i] = a[i] >> 1;
    if (!check_i32("vsra.vi", got, expected, N))
        return 0;

    count = rvv_mseq_count_first_i32(a, b, N, &first);
    if (!check_scalar("vmseq.vv/vcpop.m", count, 1))
        return 0;
    if (!check_scalar("vmseq.vv/vfirst.m", first, 6))
        return 0;

    rvv_merge_eq_i32(got, a, b, N);
    for (size_t i = 0; i < N; ++i)
        expected[i] = (i == 6) ? a[i] : b[i];
    if (!check_i32("vmerge.vvm", got, expected, N))
        return 0;
    ...
    for (size_t base = 0, remaining = N; remaining != 0;) {
        ...
        for (size_t i = 0; i < vl; ++i) {
            uint32_t element_index = index[base + i];
            expected[base + i] = element_index < vl ? a[element_index] : 0;
        }
        base += vl;
        remaining -= vl;
    }
    if (!check_i32("vrgather.vv", got, expected, N))
        return 0;

    size_t slide_vl = rvv_vsetvl_e32m1(N);
    rvv_slideup1_i32(got, a, N);
    expected[0] = 0;
    for (size_t i = 1; i < slide_vl; ++i)
        expected[i] = a[i - 1];
    if (!check_i32("vslideup.vi", got, expected, slide_vl))
        return 0;

    slide_vl = rvv_vsetvl_e32m1(N);
    rvv_slidedown1_i32(got, a, N);
    for (size_t i = 0; i + 1 < slide_vl; ++i)
        expected[i] = a[i + 1];
    expected[slide_vl - 1] = 0;
    if (!check_i32("vslidedown.vi", got, expected, slide_vl))
        return 0;

    rvv_narrow_shift_i32_to_i16_m2(got16, a, 1, N);
    for (size_t i = 0; i < N; ++i)
        expected16[i] = (int16_t)(a[i] >> 1);
    if (!check_i16("vnsra.wx", got16, expected16, N))
        return 0;

    rvv_move_scalar_pair_i32(got, a, 12345);
    if (!check_scalar("vmv.s.x/vmv.x.s", got[0], 12345))
        return 0;
